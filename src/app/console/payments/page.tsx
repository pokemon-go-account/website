...
import { Clock, CheckCircle2, XCircle, IndianRupee, Inbox } from "lucide-react";
...
export default async function PaymentsConsolePage() {
  ...
  const paymentDocs = await Payment.find({ status: "Pending" })
    .sort({ createdAt: -1 })
    .lean();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">
            Review and verify pending P2P UPI payment submissions.
          </p>
        </div>
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20">
      ...
      <div className="grid grid-cols-3 gap-px bg-zinc-200 dark:bg-white/[0.06] rounded-lg overflow-hidden border border-zinc-200 dark:border-white/[0.06]">
        {[
          ...
          { label: "Today", value: payments.filter((p) => new Date(p.createdAt).toDateString() === new Date().toDateString()).length, color: "text-violet-500" },
      ...
      {payments.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 rounded-xl border border-dashed border-zinc-200 dark:border-white/[0.06] bg-white dark:bg-[#111111] text-center space-y-3">
          ...
          <p className="text-xs text-zinc-400 dark:text-zinc-500">
            All UPI submissions have been reviewed.
          </p>
        </div>
      ) : (
        /* Payments Table */
            ...
            <table className="min-w-full divide-y divide-zinc-100 dark:divide-white/[0.04] text-sm">
              <thead>
                <tr className="bg-zinc-50 dark:bg-white/[0.02]">
                  {["Order ID", "Amount", "Customer", "UTR Number", "Submitted", "Status", "Actions"].map(
                    (h) => (
                      <th
                        key={h}
                ...
                {payments.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* UTR */}
                      <td className="px-4 py-3 font-mono text-[11px] text-violet-600 dark:text-violet-400 whitespace-nowrap font-semibold tracking-wider">
                        {p.utrNumber}
                      </td>
