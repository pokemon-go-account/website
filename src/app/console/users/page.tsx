    ...
    <div className="max-w-2xl space-y-8">
      <div className="border-b border-white/[0.05] pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">Promote Users</h1>
      ...
      <form onSubmit={handleSearch} className="flex gap-3">
        <input
          ...
          className="flex-1 h-10 px-4 bg-white/[0.02] border border-white/[0.08] rounded-xl text-white text-xs placeholder:text-zinc-600 focus:outline-none focus:border-white transition-colors"
        />
        <button
          type="submit"
          disabled={loading}
          className="h-10 px-5 rounded-xl bg-white hover:bg-zinc-200 text-black text-xs font-bold flex items-center gap-2 cursor-pointer active:scale-95 transition-all disabled:opacity-50"
        >
      ...
      {/* User Card */}
      {result && (
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.01] p-6 space-y-5">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-white font-black text-sm">{result.name || "Unnamed User"}</p>
              <p className="text-zinc-500 text-xs font-mono mt-0.5">@{result.username}</p>
              <p className="text-zinc-600 text-[10px] mt-1">{result.email}</p>
              {result.telegramUsername && (
                <p className="text-zinc-600 text-[10px]">Telegram: {result.telegramUsername}</p>
              )}
            </div>
            <div className="text-right space-y-1.5">
              <span className={cn(
                ...
                result.role === "USER" && "border-white/[0.06] bg-white/[0.02] text-zinc-400",
              )}>
                {result.role}
              </span>
              {result.isSuspended && (
          ...
          <div className="flex flex-wrap gap-2 border-t border-white/[0.05] pt-4">
            ...
            <button
              ...
              className="h-8 px-4 rounded-xl border border-white/[0.08] hover:bg-white/[0.05] text-zinc-400 hover:text-white text-[10px] font-bold flex items-center gap-1.5 cursor-pointer transition-all active:scale-95 disabled:opacity-50"
            >
