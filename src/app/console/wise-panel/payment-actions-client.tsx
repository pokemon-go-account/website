...
import { verifyWisePayment, rejectWisePayment } from "@/features/console/wise-payment-actions";

interface WisePaymentActionsProps {
  paymentId: string;
  screenshotUrl: string;
  transactionReference: string;
}

export function WisePaymentActions({ paymentId, screenshotUrl, transactionReference }: WisePaymentActionsProps) {
  ...
  const handleVerify = () => {
    ...
    startTransition(async () => {
      await verifyWisePayment(paymentId);
      setDone("Verified");
    });
  };

  const handleReject = () => {
    if (!confirm("Are you sure you want to reject this Wise payment?")) return;
    setAction("reject");
    startTransition(async () => {
      await rejectWisePayment(paymentId);
      setDone("Rejected");
    });
  };
          ...
          <div className="relative bg-white dark:bg-[#111111] rounded-2xl border border-zinc-200 dark:border-white/[0.08] p-4 max-w-lg w-full shadow-2xl space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-zinc-450 font-mono">TX: {transactionReference}</p>
              </div>
              <div className="flex items-center gap-2">
                <a
                  href={screenshotUrl}
                  download={`wise-${transactionReference}.png`}
