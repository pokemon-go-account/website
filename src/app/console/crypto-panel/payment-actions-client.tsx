...
import { verifyCryptoPayment, rejectCryptoPayment } from "@/features/console/crypto-payment-actions";

interface CryptoPaymentActionsProps {
  paymentId: string;
  screenshotUrl: string;
  txHash: string;
}

export function CryptoPaymentActions({ paymentId, screenshotUrl, txHash }: CryptoPaymentActionsProps) {
  ...
  const handleVerify = () => {
    ...
    startTransition(async () => {
      await verifyCryptoPayment(paymentId);
      setDone("Verified");
    });
  };

  const handleReject = () => {
    if (!confirm("Are you sure you want to reject this crypto payment?")) return;
    setAction("reject");
    startTransition(async () => {
      await rejectCryptoPayment(paymentId);
      setDone("Rejected");
    });
  };
          ...
          <div className="relative bg-white dark:bg-[#111111] rounded-2xl border border-zinc-200 dark:border-white/[0.08] p-4 max-w-lg w-full shadow-2xl space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-zinc-450 font-mono">TX: {txHash}</p>
              </div>
              <div className="flex items-center gap-2">
                <a
                  href={screenshotUrl}
                  download={`crypto-${txHash}.png`}
