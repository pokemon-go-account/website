...
import { Clock, CheckCircle2, XCircle, DollarSign, Inbox, Coins } from "lucide-react";
import { CryptoPaymentActions } from "./payment-actions-client";
...
export default async function CryptoPaymentsConsolePage() {
  ...
  const paymentDocs = await CryptoPayment.find({ status: "Pending" })
    .sort({ createdAt: -1 })
    .lean();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">
            Review and verify pending Cryptocurrency payment submissions.
          </p>
        </div>
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20 animate-pulse">
      ...
      <div className="grid grid-cols-3 gap-px bg-zinc-200 dark:bg-white/[0.06] rounded-lg overflow-hidden border border-zinc-200 dark:border-white/[0.06]">
        {[
          ...
          {
            label: "Total Value ($)",
            value: `$${payments.reduce((s, p) => s + p.amount, 0).toLocaleString("en-US", { minimumFractionDigits: 2 })}`,
      ...
      {payments.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 rounded-xl border border-dashed border-zinc-200 dark:border-white/[0.06] bg-white dark:bg-[#111111] text-center space-y-3">
          ...
          <p className="text-xs text-zinc-400 dark:text-zinc-500">
            All Cryptocurrency submissions have been reviewed.
          </p>
        </div>
      ) : (
        /* Payments Table */
            ...
            <table className="min-w-full divide-y divide-zinc-100 dark:divide-white/[0.04] text-sm">
              <thead>
                <tr className="bg-zinc-50 dark:bg-white/[0.02]">
                  {["Order ID", "Coin", "Amount", "Customer", "Tx Hash", "Submitted", "Status", "Actions"].map(
                    (h) => (
                      <th
                        key={h}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-zinc-100 dark:bg-white/[0.04] text-[10px] font-bold text-zinc-750 dark:text-zinc-300 border border-zinc-200 dark:border-white/[0.06]">
                      ...
                      {/* TX Hash */}
                      <td className="px-4 py-3 font-mono text-[11px] text-amber-600 dark:text-amber-400 whitespace-nowrap font-semibold tracking-wider max-w-[150px] truncate" title={p.txHash}>
                        {p.txHash}
                      </td>
