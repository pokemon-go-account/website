...
export function RegisterForm() {
  ...
  const [credState, credFormAction, isCredPending] = useActionState(registerUser, {
    success: false,
    error: null,
  });
  ...
  const handleSocialLogin = async (provider: "google" | "apple") => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setAuthError(res.error || `Failed to sign up with ${provider}.`);
      }
    } catch (err: any) {
      ...
      if (err.code === "auth/popup-closed-by-user") {
        ...
        setAuthError(err.message || `Social sign-up failed. Please try again.`);
      }
    } finally {
    ...
    <div className="w-full space-y-4 sm:space-y-5">
      ...
      {!isConfigured && (
        <div className="rounded-md border border-amber-500/30 bg-amber-50 dark:bg-amber-950/40 p-3 text-xs text-amber-800 dark:text-amber-300 flex gap-2.5 items-start">
          ...
          <p className="leading-relaxed">
            <span className="font-semibold">Firebase Keys Missing:</span> Google sign-up requires configuration in your `.env.local`.
          </p>
        </div>
      )}
      ...
      <form action={credFormAction} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          {/* Pre-submission Password Strength Indicator */}
