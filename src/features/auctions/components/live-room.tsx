...
// Sub-components (identical UI, extracted for file size)
import { LiveRoomGallery } from "./live-room/live-room-gallery";
import { LiveRoomAdminPanel } from "./live-room/live-room-admin-panel";
import { LiveRoomBiddingPanel } from "./live-room/live-room-bidding-panel";
import { LiveRoomBidHistoryModal } from "./live-room/live-room-bid-history-modal";
...
export function LiveRoom({
  ...
  const handleManualOrderChat = async (method: "Card" | "Others") => {
    ...
    try {
      ...
      if (res.success && res.orderId) {
        ...
        await addDoc(msgsRef, { text: messageText, sender: "user", senderName: username, timestamp: serverTimestamp(), read: false });
        await addDoc(msgsRef, { text: `System: Hi ${username}! Someone from our support team will reply to you here shortly to guide you through your manual payment.`, sender: "admin", senderName: "Support Team", timestamp: serverTimestamp(), read: false });
        sendChatWebhookNotification({ ticketId: chatId, ticketTitle: `Order #${orderId.substring(0, 8).toUpperCase()} (Auction Buy Now: ${methodLabel})`, senderName: username, senderType: "user", userEmail: session?.user?.email ?? undefined, text: messageText }).catch(() => {});
  ...
  useEffect(() => {
    if (!endTime) return;
    if (status === "PAUSED") { setTimeLeft("Bidding Paused"); setIsConcluded(false); return; }
    if (status === "COMPLETED") { setTimeLeft("Auction Concluded"); setIsConcluded(true); return; }
    ...
    const updateTimer = () => {
      ...
      if (diff <= 0) {
        ...
        setTimeLeft(`${daysStr}${String(hours).padStart(2, '0')}h : ${String(minutes).padStart(2, '0')}m : ${String(seconds).padStart(2, '0')}s`);
  ...
  useEffect(() => {
    if (prevBidRef.current !== activeBid) {
      ...
      setPriceFlash(activeBid > prevBidRef.current ? "up" : "down");
  ...
  const handlePlaceBid = async (amount: number) => {
    if (amount < nextMinBid) { setError(`Minimum bid requirement is $${nextMinBid}`); return; }
    setIsBidding(true);
    try { await placeBid(amount); } catch (err) { console.error("[Auction] Bid error:", err); } finally { setIsBidding(false); }
  };
  ...
  const handleViewAllClick = async () => {
    ...
    try {
      ...
      if (res.success && res.bids) { setFullBidHistory(res.bids); }
      else { setError(res.error || "Failed to load bid history."); }
    } catch (err) { setError("Failed to retrieve complete bid history."); }
  ...
  // Spec rows (for details tab)
  interface RowType { label: string; value: string | number | undefined | null; format?: (v: number) => string; }
  const specRows: RowType[] = ([
    ...
    if (typeof row.value === "string" && (row.value === "0" || row.value === "NONE")) return false;
    return true;
  });
  ...
  // Admin action handlers (passed as callbacks to AdminPanel)
  const handlePauseAuction = async () => {
    setIsAdminActionLoading(true);
    setAdminActionError(null);
    const res = await pauseAuction(auction._id);
    if (res.success) { setStatus("PAUSED"); } else { setAdminActionError(res.error || "Failed to pause auction."); }
    setIsAdminActionLoading(false);
  };
  ...
  const handleResumeAuction = async () => {
    ...
    if (res.success) { setStatus("LIVE"); } else { setAdminActionError(res.error || "Failed to resume auction."); }
    setIsAdminActionLoading(false);
  };

  const handleForceEndAuction = async () => {
    if (!confirm("Are you sure you want to end this auction immediately?")) return;
    setIsAdminActionLoading(true);
    setAdminActionError(null);
    const res = await forceEndAuction(auction._id);
    if (res.success) { setStatus("COMPLETED"); setIsConcluded(true); } else { setAdminActionError(res.error || "Failed to end auction."); }
    setIsAdminActionLoading(false);
  };

  const handleReactivateAuction = async () => {
    ...
    if (res.success) {
      setStatus("LIVE");
      setIsConcluded(false);
      setEndTime(new Date(Date.now() + hours * 60 * 60 * 1000).toISOString());
      alert("Auction successfully reactivated and status set to LIVE!");
    } else { setAdminActionError(res.error || "Failed to reactivate auction."); }
    setIsAdminActionLoading(false);
  };
  ...
  const handleDeleteAuction = async () => {
    ...
    if (res.success) { window.location.href = "/auctions"; } else { setAdminActionError(res.error || "Failed to delete auction."); setIsAdminActionLoading(false); }
  };

  // Shared bidding panel props
  const biddingPanelProps = {
    ...
    onOpenWinnerPayment: () => setIsWinnerPaymentOpen(true),
    onSetIsRegistered: setIsRegistered,
  };
  ...
  const adminPanelProps = {
    ...
    onReactivateAuction: handleReactivateAuction,
    onDeleteAuction: handleDeleteAuction,
    onOpenEdit: () => setIsAdminEditOpen(true),
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#08080a] text-zinc-900 dark:text-white py-6 pb-24 lg:pb-6 transition-colors duration-300">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="hidden lg:grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            <LiveRoomGallery screenshots={screenshots} teamColors={teamColors} team={auction.listingId.team} />
                ...
                {activeTab === "terms" && (
                  <div className="space-y-6 text-xs text-zinc-650 dark:text-zinc-305 leading-relaxed font-light animate-in fade-in duration-300">
                    <div className="space-y-2">
                      ...
                      <p>We guarantee a secure transfer pipeline. Upon auction finalization, our system admins check the account credentials, link a secure email coordinates list matching your requests, and deliver details within minutes.</p>
                    </div>
                    <div className="space-y-2">
                      <h4 className="text-sm font-bold text-zinc-800 dark:text-white uppercase tracking-wider">Refund Policy</h4>
                      <p>A 7-Day Money Back Guarantee protects you from discrepancies. If the account characteristics do not match the statistics listed on the catalog cards, you are eligible for immediate complete refunds.</p>
                    </div>
                  </div>
                )}
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={idx} className="border-b border-zinc-200 dark:border-zinc-800/80 pb-2">
                    ...
                    {openFaq === idx && (
                      <p className="text-[11px] text-zinc-500 dark:text-zinc-400 pb-3 leading-relaxed font-light">{item.a}</p>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-lg border border-zinc-200 dark:border-white/[0.06] bg-white dark:bg-[#111111] p-6 flex flex-col sm:flex-row items-center gap-6 justify-between shadow-xs">
              ...
              <Link href="/contact" className="text-[10px] text-zinc-700 dark:text-zinc-300 font-bold uppercase tracking-widest border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900 px-4 py-2 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-850 hover:text-zinc-900 dark:hover:text-white transition-colors cursor-pointer">
                Safe Trade Seal
              </Link>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="hidden lg:block mb-4">
              {isAuthorizedAdmin && <LiveRoomAdminPanel {...adminPanelProps} />}
            </div>

            {/* Desktop Bidding Control Panel */}
            <div className="hidden lg:block">
              <LiveRoomBiddingPanel {...biddingPanelProps} />
            </div>
              ...
              <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800 space-y-2">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {["PayPal", "Visa", "Mastercard", "UPI"].map((pm, idx) => (
                    <span key={idx} className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-2 py-1 rounded text-[10px] font-medium text-zinc-500 dark:text-zinc-400">{pm}</span>
                  ))}
                  ...
                  bidHistory.slice(0, 5).map((bid, idx) => (
                    <div key={bid._id} className="flex justify-between items-center py-2 border-b border-zinc-100 dark:border-zinc-800/40 text-xs">
                      <div className="space-y-0.5">
                        ...
                        <div className="text-[9px] text-zinc-500 dark:text-zinc-400">{new Date(bid.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                      </div>
      ...
      <div className="lg:hidden space-y-6 mx-auto max-w-7xl px-4 sm:px-6">
        ...
        {isAuthorizedAdmin && (
          <div className="mt-2">
            <LiveRoomAdminPanel {...adminPanelProps} />
          </div>
        )}
                      ...
                      isAuctionPaid ? (
                        <div className="rounded-xl border border-emerald-500/30 bg-gradient-to-br from-emerald-500/10 to-teal-500/5 p-4 text-center space-y-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-zinc-600 dark:text-zinc-300 mt-1">Your payment has been successfully confirmed. You will receive the account details soon!</p>
                          </div>
                          <button disabled className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl font-black uppercase text-xs tracking-wider bg-zinc-200 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500 cursor-not-allowed opacity-60">
                        ...
                        <div className="rounded-xl border border-violet-500/30 bg-gradient-to-br from-violet-500/10 to-emerald-500/5 p-4 text-center space-y-3">
                          ...
                          <button onClick={() => setIsWinnerPaymentOpen(true)} className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl font-black uppercase text-xs tracking-wider transition bg-[#6133e1] hover:bg-violet-600 text-white cursor-pointer active:scale-[0.98] shadow-lg">
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            <div className="rounded-lg border border-zinc-200 dark:border-white/[0.06] bg-white dark:bg-[#111111] p-5 space-y-6 shadow-xs">
              <div className="space-y-2">
                ...
                <p className="text-xs text-zinc-655 dark:text-zinc-300 leading-relaxed font-light whitespace-pre-line">{auction.listingId.description}</p>
              </div>
              {specRows.length > 0 && (
        ...
        {mobileActiveView === "help" && (
          <div className="space-y-4 animate-in fade-in duration-200">
            ...
            <div className="rounded-lg border border-zinc-200 dark:border-white/[0.06] bg-white dark:bg-[#111111] p-5 space-y-4 shadow-xs">
              <h4 className="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider">Shipping &amp; Handover details</h4>
              <div className="space-y-3 text-xs text-zinc-600 dark:text-zinc-300 font-light">
                <div className="flex items-center gap-2"><Check className="h-4 w-4 text-emerald-500 dark:text-emerald-400" /><span>Instant Delivery via admin transfer</span></div>
                <div className="flex items-center gap-2"><Check className="h-4 w-4 text-emerald-500 dark:text-emerald-400" /><span>Full access changeable email coordinates</span></div>
              </div>
              <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800 space-y-2">
                <span className="text-[9px] text-zinc-500 uppercase tracking-widest font-bold">Supported Payments</span>
                <div className="flex flex-wrap gap-2 pt-1">
            ...
            <div className="rounded-lg border border-zinc-200 dark:border-white/[0.06] bg-white dark:bg-[#111111] p-5 space-y-4 shadow-xs text-xs">
              ...
              <div className="space-y-3 leading-relaxed text-zinc-650 dark:text-zinc-305 font-light">
                <p><strong>Secure credentials:</strong> Complete account coordinate logins are shared securely coordinates matching your account requests.</p>
                <p><strong>Refund assurance:</strong> A 7-day money-back safeguard applies automatically if the trainer levels, shinies or levels are discrepancy-filled.</p>
              </div>
            </div>
            ...
            <div className="rounded-lg border border-zinc-200 dark:border-white/[0.06] bg-white dark:bg-[#111111] p-5 space-y-4 shadow-xs">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={idx} className="border-b border-zinc-200 dark:border-zinc-800/80 pb-2">
                    <button type="button" onClick={() => setOpenFaq(openFaq === idx ? null : idx)} className="w-full flex items-center justify-between py-2 text-left text-xs font-bold text-zinc-700 dark:text-zinc-200 hover:text-zinc-900 dark:hover:text-white cursor-pointer bg-transparent border-none">
                      <span>{item.q}</span>
                      <span>{openFaq === idx ? "−" : "+"}</span>
                    </button>
                    {openFaq === idx && <p className="text-[10px] text-zinc-500 dark:text-zinc-400 pb-2 leading-relaxed font-light">{item.a}</p>}
                  </div>
                ))}
              </div>
            </div>
      ...
      <AnimatePresence>
        {isAdminEditOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsAdminEditOpen(false)} className="absolute inset-0 bg-black/60 backdrop-blur-xs" />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="relative w-full max-w-2xl rounded-lg border border-zinc-200 dark:border-white/[0.06] bg-white dark:bg-[#111111] p-6 shadow-xl space-y-4 max-h-[90vh] flex flex-col z-10 overflow-y-auto">
              <div className="flex items-center justify-between border-b border-zinc-150 dark:border-zinc-800 pb-3">
                ...
                <button type="button" onClick={() => setIsAdminEditOpen(false)} className="h-6 w-6 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500 hover:text-zinc-800 dark:hover:text-white flex items-center justify-center text-xs font-bold bg-transparent border-none cursor-pointer">✕</button>
              </div>

              {adminActionError && (
                <div className="text-[10px] text-red-500 bg-red-500/10 p-2.5 rounded border border-red-500/20 leading-relaxed">{adminActionError}</div>
              )}

              <div className="space-y-6 text-xs max-h-[70vh] overflow-y-auto pr-2">
                {/* Core Metadata & Bidding Economics */}
                <div className="space-y-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 p-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input type="text" value={adminEditForm.title} onChange={(e) => setAdminEditForm({ ...adminEditForm, title: e.target.value })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="space-y-1">
                        <label className="text-zinc-500 font-medium">Trainer Level</label>
                        <input type="number" value={adminEditForm.level} onChange={(e) => setAdminEditForm({ ...adminEditForm, level: parseInt(e.target.value) || 0 })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                      </div>
                      <div className="space-y-1">
                        <label className="text-zinc-500 font-medium">Team Faction</label>
                        <select value={adminEditForm.team} onChange={(e) => setAdminEditForm({ ...adminEditForm, team: e.target.value as any })} className="w-full h-8 px-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none cursor-pointer">
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea value={adminEditForm.description} onChange={(e) => setAdminEditForm({ ...adminEditForm, description: e.target.value })} className="w-full min-h-[60px] p-2.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none text-[11px]" />
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      { label: "Starting Bid ($)", key: "startingBid" },
                      { label: "Reserve Price ($)", key: "reservePrice" },
                      { label: "Min Increment ($)", key: "minIncrement" },
                      { label: "Entry Deposit ($)", key: "registrationFee" },
                    ].map(({ label, key }) => (
                      <div key={key} className="space-y-1">
                        <label className="text-zinc-500 font-medium">{label}</label>
                        <input type="number" value={(adminEditForm as any)[key] || 0} onChange={(e) => setAdminEditForm({ ...adminEditForm, [key]: parseInt(e.target.value) || 0 })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                      </div>
                    ))}
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input type="datetime-local" value={new Date(new Date(adminEditForm.endTime).getTime() - new Date().getTimezoneOffset() * 60 * 1000).toISOString().slice(0, 16)} onChange={(e) => setAdminEditForm({ ...adminEditForm, endTime: new Date(e.target.value).toISOString() })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-zinc-500 font-medium">Account Region</label>
                      <input type="text" value={adminEditForm.region} onChange={(e) => setAdminEditForm({ ...adminEditForm, region: e.target.value })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                    </div>
                  </div>
                </div>

                {/* Numeric fields section - condensed grid */}
                <div className="space-y-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/55 dark:bg-zinc-900/30 p-4">
                  <h4 className="font-black text-[#6133e1] uppercase text-[10px] tracking-wider border-b border-zinc-200 dark:border-zinc-800 pb-1">Pokémon Collection Metrics</h4>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      { label: "Shiny Count", key: "shinyCount" }, { label: "Legendary Count", key: "legendaryCount" },
                      { label: "Mythical Count", key: "mythicalCount" }, { label: "Shiny Pokémon", key: "shinyPokemons" },
                      { label: "Shiny Mythical", key: "shinyMythical" }, { label: "Shiny Ultrabeasts", key: "shinyUltrabeasts" },
                      { label: "Shiny Legendaries", key: "shinyLegendaries" }, { label: "Legendary Pokémon", key: "legendaryPokemons" },
                      { label: "Ultrabeasts", key: "ultrabeasts" }, { label: "Mythical Pokémon", key: "mythicalPokemons" },
                      { label: "Hundo Myth/Leg/UB", key: "hundoMythicalLegendaryUltrabeast" }, { label: "Shundo Myth/Leg/UB", key: "shundoLegendaryMythicalUltrabeast" },
                      { label: "Shundo Pokémon", key: "shundoPokemons" }, { label: "Hundo Pokémon", key: "hundoPokemons" },
                      { label: "Costume Shinies", key: "costumeShinies" }, { label: "Hatched Shinies", key: "hatchedShinies" },
                      { label: "Location Bg Shiny Leg", key: "locationBackgroundLegendaryShiny" }, { label: "Special Bg Shiny Leg", key: "specialBackgroundLegendaryShiny" },
                      { label: "Dual Move Pokémon", key: "dualMovePokemons" }, { label: "Shadow Shiny Pokémon", key: "shadowShinyPokemons" },
                    ].map(({ label, key }) => (
                      <div key={key} className="space-y-1">
                        <label className="text-zinc-500 font-medium">{label}</label>
                        <input type="number" value={(adminEditForm as any)[key] || 0} onChange={(e) => setAdminEditForm({ ...adminEditForm, [key]: parseInt(e.target.value) || 0 })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                ...
                <div className="space-y-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 p-4">
                  <h4 className="font-black text-[#6133e1] uppercase text-[10px] tracking-wider border-b border-zinc-200 dark:border-zinc-800 pb-1 flex items-center gap-1.5"><Award className="h-3.5 w-3.5" />IV &amp; Lucky Metrics</h4>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      { label: "Lucky Pokémon", key: "luckyPokemons" }, { label: "Lucky Legendaries", key: "luckyLegendaries" },
                      { label: "Shiny Lucky Legendaries", key: "shinyLuckyLegendaries" }, { label: "Legendary Poses", key: "legendaryPoses" },
                    ].map(({ label, key }) => (
                      <div key={key} className="space-y-1">
                        <label className="text-zinc-500 font-medium">{label}</label>
                        <input type="number" value={(adminEditForm as any)[key] || 0} onChange={(e) => setAdminEditForm({ ...adminEditForm, [key]: parseInt(e.target.value) || 0 })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                ...
                <div className="space-y-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 p-4">
                  <h4 className="font-black text-[#6133e1] uppercase text-[10px] tracking-wider border-b border-zinc-200 dark:border-zinc-800 pb-1 flex items-center gap-1.5"><Coins className="h-3.5 w-3.5" />Items &amp; Storage Inventory</h4>
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    {[
                      { label: "PokeCoins", key: "pokeCoins" }, { label: "Rare Candy", key: "rareCandy" },
                      { label: "Fast TM", key: "fastTm" }, { label: "Charged TM", key: "chargedTm" },
                      { label: "Elite Fast TM", key: "eliteFastTm" }, { label: "Elite Charged TM", key: "eliteChargedTm" },
                      { label: "Incubators", key: "incubators" }, { label: "Lucky Eggs", key: "luckyEggs" },
                      { label: "Lure Modules", key: "lureModules" }, { label: "Premium Raid Pass", key: "premiumRaidPass" },
                      { label: "Master Balls", key: "masterBalls" }, { label: "Raid Passes", key: "raidPasses" },
                      { label: "Super Rocket Radar", key: "superRocketRadar" }, { label: "Pokémon Storage", key: "pokemonStorage" },
                      { label: "Item Bag Storage", key: "itemBagStorage" },
                    ].map(({ label, key }) => (
                      <div key={key} className="space-y-1">
                        <label className="text-zinc-500 font-medium">{label}</label>
                        <input type="number" value={(adminEditForm as any)[key] || 0} onChange={(e) => setAdminEditForm({ ...adminEditForm, [key]: parseInt(e.target.value) || 0 })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                ...
                <div className="space-y-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 p-4">
                  <h4 className="font-black text-[#6133e1] uppercase text-[10px] tracking-wider border-b border-zinc-200 dark:border-zinc-800 pb-1 flex items-center gap-1.5"><ShieldAlert className="h-3.5 w-3.5" />Telemetry &amp; Administration</h4>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      { label: "Total XP", key: "xp" }, { label: "Pokedex Completed %", key: "pokedexCompleted" },
                      { label: "Pokedex Reg Number", key: "pokedexRegisteredNumber" }, { label: "Platinum Medals", key: "platinumMedals" },
                      { label: "Weekly Distance (km)", key: "weeklyDistance" }, { label: "Bans/Warnings Count", key: "bansCount" },
                    ].map(({ label, key }) => (
                      <div key={key} className="space-y-1">
                        <label className="text-zinc-500 font-medium">{label}</label>
                        <input type="number" value={(adminEditForm as any)[key] || 0} onChange={(e) => setAdminEditForm({ ...adminEditForm, [key]: parseInt(e.target.value) || 0 })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                    ...
                    <div className="space-y-1">
                      ...
                      <input type="text" value={adminEditForm.startDate} onChange={(e) => setAdminEditForm({ ...adminEditForm, startDate: e.target.value })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-zinc-500 font-medium">Account Type</label>
                      <input type="text" value={adminEditForm.accountType} onChange={(e) => setAdminEditForm({ ...adminEditForm, accountType: e.target.value })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-zinc-500 font-medium">Account Status</label>
                      <input type="text" value={adminEditForm.accountStatus} onChange={(e) => setAdminEditForm({ ...adminEditForm, accountStatus: e.target.value })} className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-zinc-500 font-medium">Top Pokémon Highlights</label>
                      <input type="text" value={adminEditForm.topPokemon} onChange={(e) => setAdminEditForm({ ...adminEditForm, topPokemon: e.target.value })} placeholder="Mewtwo(3950), Kyogre(3800)" className="w-full h-8 px-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white focus:ring-1 focus:ring-[#6133e1] focus:outline-none" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-zinc-150 dark:border-zinc-800">
                <button type="button" onClick={() => setIsAdminEditOpen(false)} className="h-9 px-4 rounded-xl border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-white/5 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white font-bold cursor-pointer bg-transparent">Cancel</button>
                <button type="button" onClick={handleSaveAdminEdit} disabled={isAdminActionLoading} className="h-9 px-5 rounded-xl bg-violet-600 hover:bg-violet-500 text-white font-bold cursor-pointer disabled:opacity-50">
      ...
      {isBuyNowOpen && (
        <div
          onClick={(e) => {
            if (e.target === e.currentTarget) {
              setUpiCheckoutData(null); setPaypalCheckoutData(null);
              setCryptoCheckoutData(null); setWiseCheckoutData(null);
              setPaymentStage("methods"); setSelectedMethod(null);
              setIsBuyNowOpen(false);
            }
          }}
        ...
        >
          <div className="relative w-full max-w-6xl rounded-2xl border border-zinc-200/80 dark:border-white/[0.06] bg-white dark:bg-[#111] p-6 lg:p-8 shadow-2xl text-zinc-900 dark:text-white transition-all duration-300 cursor-default overflow-y-auto max-h-[95vh] scrollbar-thin">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative z-10 text-left">
                ...
                {paymentStage === "wise" && wiseCheckoutData ? (
                  <div className="space-y-5">
                    <div className="flex items-center justify-between pb-3 border-b border-zinc-200 dark:border-zinc-800">
                      <h3 className="font-extrabold text-sm text-zinc-900 dark:text-white flex items-center gap-1.5 uppercase tracking-wide"><Globe className="h-4.5 w-4.5 text-emerald-500" />Wise Secure Gate</h3>
                      <button onClick={() => { setWiseCheckoutData(null); setPaymentStage("methods"); setSelectedMethod(null); }} className="text-[10px] font-bold text-zinc-550 hover:text-emerald-600 cursor-pointer bg-transparent border-none transition-colors">Change Method</button>
                    </div>
                    <WisePaymentCheckout orderId={wiseCheckoutData.orderId} amount={wiseCheckoutData.amount} currency={wiseCheckoutData.currency} customerEmail={wiseCheckoutData.email} walletDiscountApplied={Math.min(buyNowPrice, walletCreditAmount)} originalTotalPrice={buyNowPrice} />
                  </div>
                ) : paymentStage === "crypto" && cryptoCheckoutData ? (
                  <div className="space-y-5">
                    <div className="flex items-center justify-between pb-3 border-b border-zinc-200 dark:border-zinc-800">
                      <h3 className="font-extrabold text-sm text-zinc-900 dark:text-white flex items-center gap-1.5 uppercase tracking-wide"><Coins className="h-4.5 w-4.5 text-amber-500" />Crypto Secure Gate</h3>
                      <button onClick={() => { setCryptoCheckoutData(null); setPaymentStage("methods"); setSelectedMethod(null); }} className="text-[10px] font-bold text-zinc-550 hover:text-amber-600 cursor-pointer bg-transparent border-none transition-colors">Change Method</button>
                    </div>
                    <CryptoPaymentCheckout orderId={cryptoCheckoutData.orderId} amount={cryptoCheckoutData.amount} customerEmail={cryptoCheckoutData.email} walletDiscountApplied={Math.min(buyNowPrice, walletCreditAmount)} originalTotalPrice={buyNowPrice} />
                  </div>
                ) : paymentStage === "paypal" && paypalCheckoutData ? (
                  <div className="space-y-5">
                    <div className="flex items-center justify-between pb-3 border-b border-zinc-200 dark:border-zinc-800">
                      <h3 className="font-extrabold text-sm text-zinc-900 dark:text-white flex items-center gap-1.5 uppercase tracking-wide"><DollarSign className="h-4.5 w-4.5 text-blue-500" />PayPal Secure Gate</h3>
                      <button onClick={() => { setPaypalCheckoutData(null); setPaymentStage("methods"); setSelectedMethod(null); }} className="text-[10px] font-bold text-zinc-505 hover:text-blue-550 dark:hover:text-blue-400 cursor-pointer bg-transparent border-none transition-colors">Change Method</button>
                    </div>
                    <PayPalPaymentCheckout orderId={paypalCheckoutData.orderId} amount={paypalCheckoutData.amount} customerEmail={paypalCheckoutData.email} walletDiscountApplied={Math.min(buyNowPrice, walletCreditAmount)} originalTotalPrice={buyNowPrice} />
                  </div>
                ) : paymentStage === "upi" && upiCheckoutData ? (
                  <div className="space-y-5">
                    <div className="flex items-center justify-between pb-3 border-b border-zinc-200 dark:border-zinc-800">
                      <h3 className="font-extrabold text-sm text-zinc-900 dark:text-white flex items-center gap-1.5 uppercase tracking-wide"><ScanQrCode className="h-4.5 w-4.5 text-[#6133e1]" />UPI Secure Gate</h3>
                      <button onClick={() => { setUpiCheckoutData(null); setPaymentStage("methods"); setSelectedMethod(null); }} className="text-[10px] font-bold text-zinc-505 hover:text-[#6133e1] dark:hover:text-purple-400 cursor-pointer bg-transparent border-none transition-colors">Change Method</button>
                    </div>
                    <UpiPaymentCheckout orderId={upiCheckoutData.orderId} amount={upiCheckoutData.amount} customerEmail={upiCheckoutData.email} walletDiscountApplied={Math.min(buyNowPrice, walletCreditAmount)} originalTotalPrice={buyNowPrice} />
                  </div>
                ) : (
                  <div className="space-y-5">
                    ...
                    {finalBuyNowPrice === 0 ? (
                      <div className="pt-4">
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              if (res.success && res.orderId) {
                                ...
                                await setDoc(chatRef, { userId, username, email: session?.user?.email ?? "", type: "order", orderId, title: `Order #${orderId.substring(0, 8).toUpperCase()}`, lastMessage: "Paid automatically via wallet credit.", lastMessageAt: serverTimestamp(), unreadByAdmin: 1, unreadByUser: 0, createdAt: serverTimestamp(), paymentMethod: "Wallet" });
                                const msgsRef = collection(db, "supportChats", chatId, "messages");
                                await addDoc(msgsRef, { text: messageText, sender: "user", senderName: username, timestamp: serverTimestamp(), read: false });
                                await addDoc(msgsRef, { text: `System: Buy Now Order #${orderId.substring(0, 8).toUpperCase()} has been successfully created and fully paid via wallet balance. Support representative will deliver your account details soon!`, sender: "admin", senderName: "Support Team", timestamp: serverTimestamp(), read: false });
                                setIsBuyNowOpen(false);
                                window.location.href = `/chat?chatId=${chatId}`;
                        ...
                        >
                          {loadingMethod === "WALLET" ? <Loader2 className="h-4 w-4 animate-spin" /> : "Confirm Wallet Payment ($0.00)"}
                        </button>
                      </div>
                    ) : (
                      <div className="space-y-3">
                        ...
                        <button
                          onClick={async () => {
                            ...
                            } catch (err) { console.error(err); alert("Failed to initiate UPI transaction. Please try again."); }
                            finally { setLoadingMethod(null); }
                          }}
                          disabled={loadingMethod !== null}
                          className={cn("flex items-center justify-between p-4 rounded-xl border border-zinc-200 dark:border-white/[0.06] bg-zinc-50 dark:bg-black/20 transition text-left w-full shadow-xs", loadingMethod !== null ? "opacity-55 cursor-not-allowed" : "hover:border-[#6133e1] dark:hover:border-[#6133e1]/50 hover:bg-white dark:hover:bg-white/[0.02] active:scale-[0.98] cursor-pointer")}
                        >
                          <div className="flex items-center gap-3">
                            <div className="flex items-center justify-center w-9 h-9 shrink-0">{loadingMethod === "UPI" ? <Loader2 className="h-5 w-5 animate-spin" /> : <ScanQrCode className="h-5 w-5 text-[#6133e1]" />}</div>
                            <div><span className="text-sm font-black text-zinc-900 dark:text-white block tracking-tight">UPI</span><span className="text-[10px] text-zinc-500 dark:text-zinc-400 font-medium">{loadingMethod === "UPI" ? "Generating UPI Gate..." : "Instant Indian payment"}</span></div>
                          </div>
                          <span className="text-[9px] bg-[#6133e1]/10 text-[#6133e1] px-2 py-0.5 rounded-full font-bold uppercase border border-[#6133e1]/10">Instant</span>
                        </button>

                        {/* Card */}
                        <button
                          onClick={async () => { setLoadingMethod("Card"); try { await handleManualOrderChat("Card"); } finally { setLoadingMethod(null); } }}
                          disabled={loadingMethod !== null}
                          className={cn("flex items-center justify-between p-4 rounded-xl border border-zinc-200 dark:border-white/[0.06] bg-zinc-50 dark:bg-black/20 transition text-left w-full shadow-xs", loadingMethod !== null ? "opacity-55 cursor-not-allowed" : "hover:border-blue-500 dark:hover:border-blue-500/50 hover:bg-white dark:hover:bg-white/[0.02] active:scale-[0.98] cursor-pointer")}
                        >
                          <div className="flex items-center gap-3">
                            <div className="flex items-center justify-center w-9 h-9 shrink-0 text-zinc-900 dark:text-white">{loadingMethod === "Card" ? <Loader2 className="h-5 w-5 animate-spin" /> : <AnimatedCardIcon className="h-5 w-5" />}</div>
                            <div><span className="text-sm font-black text-zinc-900 dark:text-white block tracking-tight">Card, Cash App, Apple Pay</span><span className="text-[10px] text-zinc-500 dark:text-zinc-400 font-medium">{loadingMethod === "Card" ? "Opening support chat..." : "Global card processing"}</span></div>
                          </div>
                        ...
                        <button
                          onClick={async () => {
                            ...
                            try {
                              const res = await createBuyNowOrderAction(auction._id);
                              if (res.success && res.orderId) { setCryptoCheckoutData({ orderId: res.orderId, amount: finalBuyNowPrice, email: session?.user?.email || "customer@store.com" }); setSelectedMethod("Crypto"); setPaymentStage("crypto"); }
                              else { alert("Error: " + (res.error || "Failed to create order")); }
                            } catch (err) { console.error(err); alert("Failed to initiate Crypto transaction. Please try again."); }
                            finally { setLoadingMethod(null); }
                          }}
                          disabled={loadingMethod !== null}
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="flex items-center justify-center w-9 h-9 shrink-0 text-zinc-900 dark:text-white">{loadingMethod === "Crypto" ? <Loader2 className="h-5 w-5 animate-spin" /> : <AnimatedCryptoIcon className="h-5 w-5" />}</div>
                            <div><span className="text-sm font-black text-zinc-900 dark:text-white block tracking-tight">Crypto</span><span className="text-[10px] text-zinc-500 dark:text-zinc-400 font-medium">{loadingMethod === "Crypto" ? "Generating Crypto Gate..." : "Secure USDT, BTC, ETH"}</span></div>
                          </div>
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              if (res.success && res.orderId) {
                                ...
                                setPaypalCheckoutData({ orderId: res.orderId, amount: Math.round(finalBuyNowPrice * eurRate * 100) / 100, email: session?.user?.email || "customer@store.com" });
                                setSelectedMethod("PayPal"); setPaymentStage("paypal");
                              } else { alert("Error: " + (res.error || "Failed to create order")); }
                            } catch (err) { console.error(err); alert("Failed to initiate PayPal transaction. Please try again."); }
                            finally { setLoadingMethod(null); }
                          }}
                          disabled={loadingMethod !== null}
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="flex items-center justify-center w-9 h-9 shrink-0 text-zinc-900 dark:text-white">{loadingMethod === "PayPal" ? <Loader2 className="h-5 w-5 animate-spin" /> : <PaypalIcon className="h-5 w-5" />}</div>
                            <div><span className="text-sm font-black text-zinc-900 dark:text-white block tracking-tight">PayPal</span><span className="text-[10px] text-zinc-500 dark:text-zinc-400 font-medium">{loadingMethod === "PayPal" ? "Generating PayPal Gate..." : "Global instant verification"}</span></div>
                          </div>
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  if (res.success && res.orderId) {
                                    ...
                                    setWiseCheckoutData({ orderId: res.orderId, amount: Math.round(finalBuyNowPrice * rate * 100) / 100, currency: selectedCurrency, email: session?.user?.email || "customer@store.com" });
                                    setSelectedMethod("Wise"); setPaymentStage("wise");
                                  } else { alert("Error: " + (res.error || "Failed to create order")); }
                                } catch (err) { console.error(err); alert("Failed to initiate Wise transaction. Please try again."); }
                                finally { setLoadingMethod(null); }
                              }}
                              disabled={isWiseDisabled || loadingMethod !== null}
                              className={cn("flex items-center justify-between p-4 rounded-xl border border-zinc-200 dark:border-white/[0.06] bg-zinc-50 dark:bg-black/20 transition text-left w-full shadow-xs", (isWiseDisabled || loadingMethod !== null) ? "opacity-55 cursor-not-allowed" : "hover:border-emerald-500 dark:hover:border-emerald-500/50 hover:bg-white dark:hover:bg-white/[0.02] active:scale-[0.98] cursor-pointer")}
                            >
                              <div className="flex items-center gap-3">
                                <div className="flex items-center justify-center w-9 h-9 shrink-0 text-zinc-900 dark:text-white">{loadingMethod === "Wise" ? <Loader2 className="h-5 w-5 animate-spin" /> : <WiseIcon className="h-5 w-5" />}</div>
                                <div><span className="text-sm font-black text-zinc-900 dark:text-white block tracking-tight">Wise</span><span className={cn("text-[10px] font-medium block", isWiseDisabled ? "text-red-500" : "text-zinc-500 dark:text-zinc-400")}>{loadingMethod === "Wise" ? "Generating Wise Gate..." : isWiseDisabled ? "Min $5.00 required" : "Direct international transfer"}</span></div>
                              </div>
                              <span className={cn("text-[9px] px-2 py-0.5 rounded-full font-bold uppercase border", isWiseDisabled ? "bg-zinc-500/10 text-zinc-400 border-zinc-500/10" : "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20")}>{isWiseDisabled ? "Disabled" : "Instant"}</span>
                            </button>
                          );
                        })()}
                        ...
                        {/* Others */}
                        <button
                          onClick={async () => { setLoadingMethod("Others"); try { await handleManualOrderChat("Others"); } finally { setLoadingMethod(null); } }}
                          disabled={loadingMethod !== null}
                          className={cn("flex items-center justify-between p-4 rounded-xl border border-zinc-200 dark:border-white/[0.06] bg-zinc-50 dark:bg-black/20 transition text-left w-full shadow-xs", loadingMethod !== null ? "opacity-55 cursor-not-allowed" : "hover:border-zinc-400 dark:hover:border-zinc-400/50 hover:bg-white dark:hover:bg-white/[0.02] active:scale-[0.98] cursor-pointer")}
                        >
                          <div className="flex items-center gap-3">
                            <div className="p-2 rounded-lg bg-zinc-500/10 text-zinc-500 dark:bg-zinc-500/20">{loadingMethod === "Others" ? <Loader2 className="h-5 w-5 animate-spin" /> : <CircleDot className="h-5 w-5" />}</div>
                            <div><span className="text-sm font-black text-zinc-900 dark:text-white block tracking-tight">Others</span><span className="text-[10px] text-zinc-500 dark:text-zinc-400 font-medium">{loadingMethod === "Others" ? "Opening support chat..." : "Payoneer, Alipay, Custom chat"}</span></div>
                          </div>
              ...
              <div className="lg:col-span-5 xl:col-span-4 bg-zinc-50/70 dark:bg-zinc-900/40 border border-zinc-200/60 dark:border-zinc-850 rounded-2xl p-5 space-y-5 lg:sticky lg:top-0">
                <h3 className="text-xs font-black text-zinc-800 dark:text-zinc-300 uppercase tracking-widest border-b border-zinc-200 dark:border-zinc-800/60 pb-2 flex items-center justify-between">
                  ...
                  <span className="font-mono text-[9px] font-normal text-zinc-400 dark:text-zinc-500 bg-zinc-100 dark:bg-zinc-800/80 px-1.5 py-0.5 rounded">VERIFIED</span>
                </h3>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="space-y-1 text-left min-w-0 flex-1">
                    <h4 className="text-xs font-bold text-zinc-900 dark:text-white truncate">{auction.listingId.title}</h4>
                    <div className="flex flex-wrap gap-1">
                      <span className="px-1.5 py-0.5 rounded text-[8px] font-black bg-zinc-100 dark:bg-zinc-800 text-zinc-650 dark:text-zinc-400 border border-zinc-200/60 dark:border-zinc-700">LVL {auction.listingId.level}</span>
                      {auction.listingId.team !== "NONE" && (
                        <span className={cn("px-1.5 py-0.5 rounded text-[8px] font-black border uppercase", teamColors[auction.listingId.team])}>{auction.listingId.team}</span>
                      )}
                    </div>
                ...
                <div className="space-y-2.5 text-xs border-y border-zinc-200 dark:border-zinc-800/60 py-3 font-medium">
                  <div className="flex justify-between text-zinc-555 dark:text-zinc-400"><span>Listing Buy Now Price</span><span className="text-zinc-855 dark:text-zinc-200 font-bold"><PriceDisplay amountInUSD={buyNowPrice} /></span></div>
                  {hasWalletCredit && (<div className="flex justify-between text-zinc-555 dark:text-zinc-400"><span>Wallet Credit Applied</span><span className="text-emerald-500 font-bold">-<PriceDisplay amountInUSD={buyNowPriceDiscount} /></span></div>)}
                  <div className="flex justify-between text-zinc-555 dark:text-zinc-400"><span>Secure Protection</span><span className="text-emerald-600 dark:text-emerald-450 font-black uppercase text-[9px] bg-emerald-500/10 border border-emerald-500/20 px-1.5 py-0.5 rounded">Free</span></div>
                  <div className="flex justify-between text-zinc-555 dark:text-zinc-400"><span>Estimated Tax / VAT</span><span className="text-zinc-800 dark:text-zinc-300 font-bold">$0.00</span></div>
                </div>
                ...
                <div className="pt-3 border-t border-zinc-200 dark:border-zinc-800/60 space-y-2.5 text-left">
                  <div className="flex items-start gap-2">
                    ...
                    <div><p className="text-[10px] font-bold text-zinc-800 dark:text-zinc-200">Buyer Protection Guarantee</p><p className="text-[9px] text-zinc-450 dark:text-zinc-500 leading-normal">Full protection. Funds are held secure until you verify the account.</p></div>
                  </div>
                  <div className="flex items-start gap-2">
                    <Sparkles className="h-4.5 w-4.5 text-violet-500 shrink-0 mt-0.5" />
                    <div><p className="text-[10px] font-bold text-zinc-800 dark:text-zinc-200">Anti-Recall Warranty</p><p className="text-[9px] text-zinc-450 dark:text-zinc-500 leading-normal">Every account passes rigorous safety logs and features a recall warrant.</p></div>
                  </div>
                </div>
              </div>
            </div>
      ...
      {isWinnerPaymentOpen && (() => {
        ...
        const closeWinnerModal = () => {
          setWinnerUpiCheckoutData(null); setWinnerPaypalCheckoutData(null);
          setWinnerCryptoCheckoutData(null); setWinnerWiseCheckoutData(null);
          setWinnerPaymentStage("methods"); setIsWinnerPaymentOpen(false);
        };
        ...
        const handleWinnerManualChat = async (method: "Card" | "Others") => {
          ...
          try {
            ...
            if (res.success && res.orderId) {
              ...
              await setDoc(chatRef, { userId, username, email: session?.user?.email ?? "", type: "order", orderId, title: `Auction Win #${orderId.substring(0, 8).toUpperCase()}`, lastMessage: `Auction win payment started via ${methodLabel}.`, lastMessageAt: serverTimestamp(), unreadByAdmin: 1, unreadByUser: 0, createdAt: serverTimestamp() });
              const msgsRef = collection(db, "supportChats", chatId, "messages");
          ...
          } catch (err) { console.error(err); alert("Failed to initiate support chat. Please try again."); }
        };

        return (
          <div onClick={(e) => { if (e.target === e.currentTarget) closeWinnerModal(); }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-200 cursor-pointer">
            <div className="relative w-full max-w-lg rounded-2xl border border-violet-500/20 bg-white dark:bg-[#111] p-6 shadow-2xl text-zinc-900 dark:text-white transition-all duration-300 cursor-default overflow-y-auto max-h-[90vh]">
              ...
              <button onClick={closeWinnerModal} className="absolute top-4 right-4 text-zinc-400 hover:text-zinc-950 dark:hover:text-white cursor-pointer bg-transparent border-none z-10"><X className="h-5 w-5" /></button>

              <div className="space-y-5 relative z-10">
                <div className="space-y-1 border-b border-zinc-200 dark:border-zinc-800 pb-4">
                  <div className="flex items-center gap-2"><Trophy className="h-5 w-5 text-[#6133e1]" /><h2 className="text-base font-black uppercase tracking-tight text-zinc-950 dark:text-white">Complete Auction Payment</h2></div>
                  <p className="text-xs text-zinc-500 dark:text-zinc-400">{auction.listingId.title} — Winning Bid: <strong className="text-zinc-700 dark:text-zinc-200"><PriceDisplay amountInUSD={winAmount} /></strong></p>
                </div>

                <div className="bg-zinc-50 dark:bg-zinc-900/60 rounded-xl p-4 space-y-2 text-xs">
                  <div className="flex justify-between text-zinc-500 dark:text-zinc-400"><span>Winning Bid</span><span className="font-bold text-zinc-800 dark:text-zinc-200"><PriceDisplay amountInUSD={winAmount} /></span></div>
                  {winDiscount > 0 && (<div className="flex justify-between text-zinc-500 dark:text-zinc-400"><span>Wallet Credit</span><span className="font-bold text-emerald-500">−<PriceDisplay amountInUSD={winDiscount} /></span></div>)}
                  <div className="flex justify-between font-black text-sm pt-1 border-t border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-white"><span>Total Due</span><span className="text-[#6133e1]"><PriceDisplay amountInUSD={finalWinPrice} /></span></div>
                </div>

                {winnerPaymentStage === "wise" && winnerWiseCheckoutData ? (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between"><h3 className="font-extrabold text-sm flex items-center gap-1.5"><Globe className="h-4 w-4 text-emerald-500" />Wise Gate</h3><button onClick={() => { setWinnerWiseCheckoutData(null); setWinnerPaymentStage("methods"); }} className="text-[10px] font-bold text-zinc-500 hover:text-emerald-600 cursor-pointer bg-transparent border-none">Change Method</button></div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between"><h3 className="font-extrabold text-sm flex items-center gap-1.5"><Coins className="h-4 w-4 text-amber-500" />Crypto Gate</h3><button onClick={() => { setWinnerCryptoCheckoutData(null); setWinnerPaymentStage("methods"); }} className="text-[10px] font-bold text-zinc-500 hover:text-amber-600 cursor-pointer bg-transparent border-none">Change Method</button></div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between"><h3 className="font-extrabold text-sm flex items-center gap-1.5"><DollarSign className="h-4 w-4 text-blue-500" />PayPal Gate</h3><button onClick={() => { setWinnerPaypalCheckoutData(null); setWinnerPaymentStage("methods"); }} className="text-[10px] font-bold text-zinc-500 hover:text-blue-600 cursor-pointer bg-transparent border-none">Change Method</button></div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between"><h3 className="font-extrabold text-sm flex items-center gap-1.5"><ScanQrCode className="h-4 w-4 text-[#6133e1]" />UPI Gate</h3><button onClick={() => { setWinnerUpiCheckoutData(null); setWinnerPaymentStage("methods"); }} className="text-[10px] font-bold text-zinc-500 hover:text-violet-600 cursor-pointer bg-transparent border-none">Change Method</button></div>
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (res.success && res.orderId) {
                          ...
                          const db = getDb();
                          const chatId = `order-${orderId}`;
                          await setDoc(doc(db, "supportChats", chatId), { userId, username, email: session?.user?.email ?? "", type: "order", orderId, title: `Auction Win #${orderId.substring(0, 8).toUpperCase()}`, lastMessage: "Paid via wallet balance.", lastMessageAt: serverTimestamp(), unreadByAdmin: 1, unreadByUser: 0, createdAt: serverTimestamp() });
                          const msgsRef = collection(db, "supportChats", chatId, "messages");
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {/* PayPal */}
                    <button onClick={async () => { setWinnerLoadingMethod("PayPal"); try { const res = await createAuctionWinnerOrderAction(auction._id); if (res.success && res.orderId) { setWinnerPaypalCheckoutData({ orderId: res.orderId, amount: finalWinPrice, email: session?.user?.email || "" }); setWinnerPaymentStage("paypal"); } else { alert(res.error || "Failed"); } } catch (err) { console.error(err); } finally { setWinnerLoadingMethod(null); } }} disabled={winnerLoadingMethod !== null} className="flex flex-col items-center gap-2 p-4 rounded-xl border border-zinc-200 dark:border-white/[0.06] bg-zinc-50 dark:bg-black/20 hover:border-blue-500 hover:bg-white dark:hover:bg-white/[0.02] active:scale-[0.98] cursor-pointer transition disabled:opacity-50">
                    ...
                    {/* Crypto */}
                    <button onClick={async () => { setWinnerLoadingMethod("Crypto"); try { const res = await createAuctionWinnerOrderAction(auction._id); if (res.success && res.orderId) { setWinnerCryptoCheckoutData({ orderId: res.orderId, amount: finalWinPrice, email: session?.user?.email || "" }); setWinnerPaymentStage("crypto"); } else { alert(res.error || "Failed"); } } catch (err) { console.error(err); } finally { setWinnerLoadingMethod(null); } }} disabled={winnerLoadingMethod !== null} className="flex flex-col items-center gap-2 p-4 rounded-xl border border-zinc-200 dark:border-white/[0.06] bg-zinc-50 dark:bg-black/20 hover:border-amber-500 hover:bg-white dark:hover:bg-white/[0.02] active:scale-[0.98] cursor-pointer transition disabled:opacity-50">
                    ...
                    {/* Wise */}
                    <button onClick={async () => { if (isWinnerWiseDisabled) return; setWinnerLoadingMethod("Wise"); try { const res = await createAuctionWinnerOrderAction(auction._id); if (res.success && res.orderId) { const cur = useCurrencyStore.getState().currency; const rate = useCurrencyStore.getState().rates[cur] || 1; setWinnerWiseCheckoutData({ orderId: res.orderId, amount: Math.round(finalWinPrice * rate * 100) / 100, currency: cur, email: session?.user?.email || "" }); setWinnerPaymentStage("wise"); } else { alert(res.error || "Failed"); } } catch (err) { console.error(err); } finally { setWinnerLoadingMethod(null); } }} disabled={isWinnerWiseDisabled || winnerLoadingMethod !== null} className={cn("flex flex-col items-center gap-2 p-4 rounded-xl border border-zinc-200 dark:border-white/[0.06] bg-zinc-50 dark:bg-black/20 transition", isWinnerWiseDisabled ? "opacity-50 cursor-not-allowed" : "hover:border-emerald-500 hover:bg-white dark:hover:bg-white/[0.02] active:scale-[0.98] cursor-pointer")}>
                    ...
                    {/* Card */}
                    <button onClick={async () => { setWinnerLoadingMethod("Card"); try { await handleWinnerManualChat("Card"); } finally { setWinnerLoadingMethod(null); } }} disabled={winnerLoadingMethod !== null} className="flex flex-col items-center gap-2 p-4 rounded-xl border border-zinc-200 dark:border-white/[0.06] bg-zinc-50 dark:bg-black/20 hover:border-zinc-400 hover:bg-white dark:hover:bg-white/[0.02] active:scale-[0.98] cursor-pointer transition disabled:opacity-50">
                    ...
                    {/* Others */}
                    <button onClick={async () => { setWinnerLoadingMethod("Others"); try { await handleWinnerManualChat("Others"); } finally { setWinnerLoadingMethod(null); } }} disabled={winnerLoadingMethod !== null} className="flex flex-col items-center gap-2 p-4 rounded-xl border border-zinc-200 dark:border-white/[0.06] bg-zinc-50 dark:bg-black/20 hover:border-zinc-400 hover:bg-white dark:hover:bg-white/[0.02] active:scale-[0.98] cursor-pointer transition disabled:opacity-50">
