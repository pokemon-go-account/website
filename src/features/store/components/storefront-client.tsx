...
export function StorefrontClient({ categories, products, initialCategorySlug }: StorefrontClientProps) {
  ...
  useEffect(() => {
    ...
    if (productId && products.length > 0) {
      const foundProduct = products.find((p) => (p._id?.toString() ?? p._id) === productId);
      if (foundProduct) {
        console.log(`[Storefront] 🛒 Opening Specific Product Modal -> "${foundProduct.name}" (ID: ${productId})`);
        setSelectedProduct(foundProduct);
        if (foundProduct.categoryId) {
          const catId = typeof foundProduct.categoryId === "object" ? (foundProduct.categoryId as any)._id : foundProduct.categoryId;
          if (catId) {
