...
Please verify my payment proof and approve my order!`;
        ...
        if (recoveryItem && recoveryItem.recoveryRequestId) {
          ...
          await setDoc(recChatRef, {
            lastMessage: `Payment proof submitted (${selectedCoin.name}). Status: In Progress.`,
            lastMessageAt: serverTimestamp(),
            unreadByAdmin: 1,
          }, { merge: true });
          ...
          await addDoc(recMsgsRef, {
            text: `📦 RECOVERY PAYMENT SUBMITTED (Crypto: ${selectedCoin.name})
----------------------------------
Recovery Request ID: ${recoveryItem.recoveryRequestId}
Amount Paid: $${amount.toLocaleString("en-US", { minimumFractionDigits: 2 })} USD
Selected Coin: ${selectedCoin.name} (${selectedCoin.network})
Order ID: ${orderId}
